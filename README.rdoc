...
The MarkUs Project practices pre-commit code review with {ReviewBoard}[http://www.reviewboard.org] in order to reduce the number of 
...
TestDrive will do the job of checking out a fresh copy of MarkUs, grabbing the diff up for review from ReviewBoard,
...
Every review request up on ReviewBoard has a unique ID.  You can see this ID in the URL when you're viewing the
...
Secondly, take a look at prepare_markus.sh.  This is the shell script that is run to "prepare" MarkUs before
applying the diff.  If you have any special requirements for set up, this is where they should go.
...
You'll also need the {JSON Ruby gem}[http://flori.github.com/json/]
...
Thanks to {Severin}[http://www.jerboaa.org] for the help polishing it up!
